...
f=[500:10:2000]; % frequencies to evaluate
...
% -- Outputs -- 
addAnalogOutputChannel(s,'Dev1',0,'Voltage'); % cos ouput
addAnalogOutputChannel(s,'Dev1',1,'Voltage'); % sin output

% -- Inputs -- 
ch1 = addAnalogInputChannel(s,'Dev2',0,'Voltage'); % mic 1
ch2 = addAnalogInputChannel(s,'Dev2',1,'Voltage'); % mic 2
ch3 = addAnalogInputChannel(s,'Dev2',2,'Voltage'); % mic 3
ch4 = addAnalogInputChannel(s,'Dev2',3,'Voltage'); % mic 4
ch5 = addAnalogInputChannel(s,'Dev2',4,'Voltage'); % cos input
ch6 = addAnalogInputChannel(s,'Dev2',5,'Voltage'); % sin input
ch1.Range = [-.5 .5];
ch2.Range = [-.5 .5];
ch3.Range = [-.5 .5];
ch4.Range = [-.5 .5];
...
for n=1:length(f) %like or freq in freqrange forloop
    for m=1:4 %probs each mic
        ...
        nta=fix(.3*(Nc(n+1))); %yeah lost cuz idk what Nc is
        ntb=fix(.8*(Nc(n+1)));
